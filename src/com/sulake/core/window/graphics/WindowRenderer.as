   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(this.getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            this.var_338.x = param2.x - param1.x;
            this.var_338.y = param2.y - param1.y;
            this.var_338.width = param2.width;
            this.var_338.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_33))
            {
               while(param1.testParamFlag(WindowParam.const_33))
               {
                  ...
                  this.var_338.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            this.getWindowRendererItem(param1).invalidate(param1,this.var_338,WindowRedrawFlag.const_1212);
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            ...
            param1.addEventListener(WindowDisposeEvent.const_968,this.windowDisposedCallback);
         }
      }
      
      public function removeRenderable(param1:IWindow) : void
      {
         param1.removeEventListener(WindowDisposeEvent.const_968,this.windowDisposedCallback);
         var _loc2_:WindowRendererItem = this.var_383[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            ...
            if(!_loc1_.disposed)
            {
               ...
               this.var_338.x = _loc3_.dirty.x;
               this.var_338.y = _loc3_.dirty.y;
               this.var_338.width = _loc3_.dirty.width;
               this.var_338.height = _loc3_.dirty.height;
               this.renderWindowBranch(_loc1_,this.var_338,_loc1_.rectangle.clone());
            }
            _loc4_++;
         }
      }
      ...
      private function renderWindowBranch(param1:WindowController, param2:Rectangle, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param2,this.var_1887,this.var_1518))
            {
               if(this.getWindowRendererItem(param1).render(param1,this.var_1887,this.var_1518))
               {
