package com.sulake.core.utils
{
   public class ErrorReportStorage
   {
      ...
      private static var var_1338:Map = new Map();
      
      private static var var_886:Map = new Map();
      ...
      public static function getDebugData() : String
      {
         var _loc1_:String = "";
         var _loc2_:int = 0;
         while(_loc2_ < var_886.length)
         {
            if(_loc2_ == 0)
            {
               _loc1_ = var_886.getWithIndex(_loc2_);
            }
            else
            {
               _loc1_ = _loc1_ + " ** " + var_886.getWithIndex(_loc2_);
            }
            _loc2_++;
         }
