package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_490.getValue(_loc4_) == null)
               {
                  this.var_490.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         var _loc3_:IPetData = this.var_490.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      public function createPetFigureString(param1:int, param2:int, param3:uint) : String
      {
         ...
         var _loc4_:PetData = this.var_490.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_490.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_490.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.const_679.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1637.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_710.getValue(_loc6_) == null)
               {
                  ...
                  this.var_710.add(_loc6_,_loc8_);
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_710.getKeys())
         {
            _loc5_ = this.var_710.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
