package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_389 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_671 && this.var_1551)
         {
            _loc5_ = this.var_27.manager.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_113,this.var_1034,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_42,parseInt(FigureData.const_964));
         ...
         if(this.var_389)
         {
            if(this.var_389.bitmap)
            {
               this.var_389.bitmap.dispose();
            }
            this.var_389.bitmap = new BitmapData(this.var_389.width,this.var_389.height,true,0);
            _loc7_ = (this.var_389.width - _loc1_.width) / 2;
            _loc8_ = (this.var_389.height - _loc1_.height) / 2;
            this.var_389.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_671 = this.var_27.figureData.getFigureString();
               this.var_1034 = this.var_27.gender;
               this.var_27.handler.saveWardrobeOutfit(this.var_1931,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_671)
               {
                  this.var_27.loadAvatarInEditor(this.var_671,this.var_1034,this.var_27.clubMemberLevel);
               }
         }
      }
