package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_791:IFigurePartSet;
      ...
      private const const_2372:int = 13421772;
      
      private const const_2373:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_757 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1787 = Math.max(this.var_1787,_loc6_.colorLayerIndex);
            }
         }
         this.var_449 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_757 != null && !this.var_1786)
            {
               _loc3_ = this.var_757;
            }
            else
            ...
            if(this.var_791)
            {
               switch(this.var_791.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_43:
                     _loc2_.style = HabboIconType.const_1016;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1481.color = this.const_2372;
         }
         else
         {
            this.var_1481.color = this.const_2373;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_449.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_449.isFigureReady(_loc1_))
         {
            this.var_449.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_387[_loc6_] + "_" + FigureData.const_164;
               _loc4_ = this.var_449.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_636 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1788)
               {
                  ...
                  this.var_1788 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_636.width,this.var_636.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_636.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_636.y;
