package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2874:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2874 = new RoomThumbnailRenderer(param1);
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_210 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_210.visible = true;
         Util.hideChildren(this.var_210);
         this.refreshRoomName(this.var_210,this.var_142);
         if(this.var_142.ownerName != "" && this.var_142.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_210,!!this.var_142.event ? this.getEventCtg(this.var_142.categoryId) : this.getRoomCtg(this.var_142.categoryId));
         this.refreshRoomDesc(this.var_210,this.var_142);
         this.refreshExtraCont();
         this.refreshInfo(this.var_210,"trading_allowed",this.var_142.allowTrading);
         this.refreshInfo(this.var_210,"doormode_doorbell",this.var_142.doorMode == 1);
         this.refreshInfo(this.var_210,"doormode_password",this.var_142.doorMode == 2);
         Util.moveChildrenToColumn(this.var_210,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_210.height = Util.getLowestPoint(this.var_210);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_210,this.var_142.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_210.findChildByName("roomowner"));
         if(this.var_142.ownerName != "" && this.var_142.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_210.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1049.refreshTags(_loc1_,this.var_142.tags);
         ...
         this.var_2874.refreshThumbnail(_loc1_,this.var_142.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_265,Tabs.const_415,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
