package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_849:String = "InitConnection";
       
      
      private var var_133:HabboCommunicationDemo;
      ...
      {
         this.var_133.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_133 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_133.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_164,HabboWindowStyle.const_47,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_133.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         if(_loc8_)
         {
            if(this.var_133.habboConfiguration)
            {
               if(this.var_133.habboConfiguration.getKey("use.sso","false") == "true")
               {
                  _loc8_.select();
               }
            }
         ...
         if(this.var_133.habboConfiguration && this.var_133.habboConfiguration.getKey("try.existing.session","false") == "true")
         {
            this.var_1526 = true;
            this._window.visible = false;
            this.windowEventProcessor(WindowEvent.allocate(WindowEvent.const_147,this._window,null,false));
         }
      }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_133.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_133.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         {
            dispatchEvent(new Event(const_849));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_133.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         if(this.var_155 != null)
         {
            ...
            this.var_155.removeEventListener(SSOTicket.const_322,this.onParsedTicketFailure);
            this.var_155 = null;
         }
         var _loc2_:String = "";
         if(this.var_133.habboConfiguration != null)
         {
            _loc2_ = this.var_133.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_133.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_155 = new SSOTicket(this.var_133.assets,_loc3_,_loc2_,param1);
         this.var_155.addEventListener(SSOTicket.const_399,this.onParsedTicketSuccess);
         this.var_155.addEventListener(SSOTicket.const_322,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_133.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_133.flashClientUrl = this.var_155.flashClientUrl;
         this.var_133.ssoTicket = this.var_155.ticket;
         this.var_155.removeEventListener(SSOTicket.const_399,this.onParsedTicketSuccess);
         this.var_155.removeEventListener(SSOTicket.const_322,this.onParsedTicketFailure);
         this.var_155 = null;
         dispatchEvent(new Event(const_849));
