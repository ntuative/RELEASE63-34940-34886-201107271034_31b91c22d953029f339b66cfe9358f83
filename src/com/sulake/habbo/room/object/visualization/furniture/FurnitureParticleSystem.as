package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_303:IRoomObjectSprite;
      ...
      private var var_1361:Number = 1.0;
      
      private var var_725:BitmapData;
      
      private var var_900:ColorTransform;
      
      private var var_1667:ColorTransform;
      
      private var var_2124:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_900 = new ColorTransform();
         this.var_900.alphaMultiplier = 1;
         this.var_1667 = new ColorTransform();
         this.var_2124 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_116 = this.var_1360[param1];
         this.var_1112 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2125 >= 0)
         {
            this.var_303 = this._visualization.getSprite(this.var_2125);
            if(this.var_303 && this.var_303.asset)
            {
               if(this.var_303.width <= 1 || this.var_303.height <= 1)
               {
                  return;
               }
               if(this.var_101 == null)
               {
                  this.var_101 = this.var_303.asset.clone();
                  if(this.var_900.alphaMultiplier != 1)
                  {
                     this.var_725 = new BitmapData(this.var_101.width,this.var_101.height,true,4278190080);
                  }
               }
               this.var_2378 = -this.var_303.offsetX;
               this.var_2377 = -this.var_303.offsetY;
               this.var_303.asset = this.var_101;
            }
            if(this.var_101)
            {
            ...
            if(this.var_725)
            {
               this.var_725.fillRect(this.var_725.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_116.y * this.var_1361;
         }
         return 0;
      }
      ...
      {
         if(!this.var_116 || !this.var_303)
         {
            return;
         }
         if(this.var_101 && this.var_303.asset != this.var_101)
         {
            this.var_303.asset = this.var_101;
         }
         if(this.var_1112)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1112 && this.var_116.hasIgnited)
         {
            this.var_1112 = true;
         }
         _loc8_ = this.var_1169 * this.var_1361;
         this.var_116.update();
         if(this.var_1112)
         {
            if(this.var_116.roomObjectSpriteId >= 0)
            ...
            {
               this.var_101.draw(this.var_725,this.var_2124,this.var_900,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_116.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2378 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1361;
               _loc4_ = this.var_2377 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1361;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1667.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_101.draw(_loc7_,this._translationMatrix,this.var_1667,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2913 = parseInt(param1.@size);
         this.var_2125 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1169 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1361 = this.var_2913 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_900.alphaMultiplier = this._blend;
