package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_388.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_388.add(_loc3_.type,_loc1_);
               this.var_63.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_388.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_388.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_63.removeListItem(_loc1_.window);
               this.var_388.remove(this.var_388.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_63.scrollableRegion.height;
         this.var_63.height = Math.max(Math.min(_loc5_,const_1580),const_1579);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1399,true);
      }
   }
}
