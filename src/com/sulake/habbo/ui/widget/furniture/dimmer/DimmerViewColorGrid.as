package com.sulake.habbo.ui.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_595.numGridItems)
         {
            return;
         }
         this.select(this.var_595.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_595 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_595.destroyGridItems();
         this.var_237 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_2148) as IWindowContainer;
            ...
            _loc3_.width = this.var_738.width;
            _loc3_.height = this.var_738.height;
            this.var_595.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_738.width,this.var_738.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_738,this.var_738.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1370.width,this.var_1370.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_1124.width,this.var_1124.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_1124,this.var_1124.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_595.getGridItemIndex(param1.target as IWindow);
