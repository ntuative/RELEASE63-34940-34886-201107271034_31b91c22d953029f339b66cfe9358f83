package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1565:String = "%";
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. Failed to load URL " + this.getKey("external.variables.txt") + "HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1319);
      }
      ...
      private function onInitConfiguration(param1:Event = null) : void
      {
         ...
         if(_loc3_ == null || _loc3_.length == 0)
         {
            ...
            Core.error("Could not load external variables, got empty data from URL " + this.getKey("external.variables.txt") + " data length = " + _loc5_ + ". Client startup failed!",false,Core.const_1319);
         }
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false, param4:Boolean = false) : void
      {
         ...
         if(this.var_1991.indexOf(param1) < 0 || param3)
         {
            this.var_866[param1] = param2;
         }
         if(param3)
         {
            this.var_1991.push(param1);
         }
      }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1565);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1565,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
