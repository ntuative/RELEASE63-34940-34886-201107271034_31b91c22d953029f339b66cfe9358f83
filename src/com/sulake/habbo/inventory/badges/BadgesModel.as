package com.sulake.habbo.inventory.badges
{
   ...
   public class BadgesModel implements IInventoryModel
   {
      ...
      public function BadgesModel(param1:HabboInventory, param2:IHabboWindowManager, param3:IHabboCommunicationManager, param4:IAssetLibrary, param5:ISessionDataManager)
      {
         ...
         this.var_415 = param5;
         this.var_415.events.addEventListener(BadgeImageReadyEvent.const_230,this.onBadgeImageReady);
      ...
      public function dispose() : void
      {
         ...
         {
            this.var_415.events.removeEventListener(BadgeImageReadyEvent.const_230,this.onBadgeImageReady);
            this.var_415 = null;
         }
      }
      ...
      private function stopWearingBadge(param1:Badge) : void
      {
         ...
         {
            if(this.var_857[_loc2_] == param1)
            {
               this.var_857.splice(_loc2_,1);
      ...
      public function updateBadge(param1:String, param2:Boolean, param3:int = 0) : void
      {
         ...
         if(param3 > 0)
         {
            this.var_1272.add(param1,param3);
         }
         ...
         else
         {
            ...
            _loc10_ = this.var_676.indexOf(param3) != -1;
            _loc11_ = new Badge(param1,_loc7_,_loc10_);
            _loc11_.iconImage = this.var_415.getBadgeImage(param1);
            if(_loc10_)
            {
               this.var_250.unshift(_loc11_);
            }
            else
            {
               this.var_250.push(_loc11_);
            }
            if(param2)
            {
      ...
      public function setBadgeSelected(param1:String) : void
      {
         ...
         while(_loc2_ < this.var_250.length)
         {
            _loc3_ = this.var_250[_loc2_] as Badge;
            if(_loc3_ != null)
            {
      ...
      public function forceSelection() : void
      {
         ...
         var _loc2_:Array = this.getBadges(const_617);
      ...
      public function getBadges(param1:int = -1) : Array
      {
         ...
         switch(param1)
         {
            ...
            case const_617:
               _loc2_ = new Array();
               for each(_loc3_ in this.var_250)
               {
                  if(!_loc3_.isInUse)
                  {
                     _loc2_.push(_loc3_);
      ...
      {
         return this.getItemInIndex(param1,const_617);
      }
      ...
      public function addUnseenBadges(param1:Array) : void
      {
         ...
         {
            if(this.var_676.indexOf(_loc2_) == -1)
            {
               this.var_676.push(_loc2_);
            }
         }
      }
      ...
      public function resetUnseenItems() : void
      {
         ...
         if(this.var_676 && this.var_676.length > 0)
         {
            _loc1_.send(new ResetUnseenItemsComposer(UnseenItemCategoryEnum.const_1254));
            this.var_676 = [];
            for each(_loc2_ in this.var_250)
            {
